...
define(function(require) {
	...
	function goday(y,m,d,type){
		var thisy,thism,thisd,thistime;
		if(y!=''&&m!=''&&d!=''){
			thisy=y;
			thism=m;
			thisd=d;
			...
			var y_m=$("#this_onedate").html().split(" ")[0].split("-");
			thisy=Number(y_m[0]);
			thism=Number(y_m[1]);
			thisd=Number(y_m[2]);
			thistime=$("#this_onedate").html().split(" ")[1];
			if(type=="prv"){
				thisd--;
				if(thisd<1){
					thism--;	
		...
		$("#this_onedate").html(thisy+"-"+thism+"-"+thisd+" "+thistime);
		var da=tommor(thisy,thism,thisd);
		$("#this_twodate").html(da[0]+"-"+da[1]+"-"+da[2]+" "+thistime);
		
		/**执行表格数据变化s**/
		/**执行表格数据变化e**/
	}
	
	function tommor(y,m,d){
		d++;
		if(d>DayNumOfMonth(y,m)){
			d=1;
			m++;if(m>12){y++;m=1;}else{y=y;m=m;}
		}else{
			d=d;y=y;m=m;
		}
		var date=[y,m,d];
