...
define(function(require) {
	...
	var thishtml='<div class="col-sm-12 mb10"><table class="table table-bordered table_card"><tbody><tr><td colspan="2" class="col-xs-10"><h4 class="mt0 mb0">';
	...
	var thishtml3='</td></tr></tbody></table></div>';
	var thishtml4='';
	
	//渲染表格		  
	drawhtml=function(id,data){
		for(var key in data){
			id.append(thishtml+key+thishtml2+data[key][0]+thishtml3);
		}				
	}
	...
	modaldraw=function(id,data,keys){
		...
		if(isNull(keys)!="kong"){
			...
			//$("#cadr_mould .jl").attr("placeholder",data[keys][1]).parents(".form-group").show();
			...
			for(var key in data){
				...
				//$("#cadr_mould .jl").attr("placeholder",data[key][1]).parents(".form-group").hide()
				id.append($("#cadr_mould").html());
			}
		}
