	...
	$.fn.extend({
        "qkynav": function (options) {
            ...
            return this.each(function (i) {
				...
				tofor($("#navbar .nav"),opts.navli,"lia");
				...
				$(".navbtn").on("click",function(){$(".nav_more").slideToggle(300);});
		    })
		}
	});
	
	
	tofor = function(id,data_arr,type) {
		for(var i=0;i<data_arr.length;i++){
			if(type=="lia"){	
			id.append("<li><a "+data_arr[i][1]+">"+data_arr[i][0]+"</a></li>");
			}else{
			id.append("<a "+data_arr[i][1]+">"+data_arr[i][0]+"</a>");
			}
		}
	};
