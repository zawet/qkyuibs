...
define(function(require) {
	...
	function table_calendar(y,m,d,id){
				...
				weekHtml+='<td class="oldday">&nbsp;</td>';
					...
					if(thisday>md){
						...
						if(thisday==d&&y==toy&&m==tom+1){
							weekHtml+='<td><div date="'+y+'-'+m+'-'+thisday+'" class="calday today"><label>'+thisday+'</label></div></td>';
						}else{
							weekHtml+='<td><div date="'+y+'-'+m+'-'+thisday+'" class="calday"><label>'+thisday+'</label></div></td>';
						}
					}
				}
			}
		...
		id.find(".calday .more").on("click",function(){$(this).parent().find(".morebox").slideDown(200);})
